...
class ReferFriendViewController: BaseViewController {
    ...
    func getJobsList1()
    {
        if ConnectionCheck.isConnectedToNetwork()
        {
            ...
            let params:[String:String] = ["JobCategory":"","DivisionId":UserDefaults.standard.object(forKey: "dID") as! String]
            
            print(params)
            ServerService.referalGetJobCategoryList(self, params:params, method:"POST", accessToken:Constants.Token, acces:true, callBack:self.getresponseForJobsList1(response:))
        }
        else
        {
    ...
    func getresponseForJobsList1(response:AnyObject)->()
    {
        jobs.removeAll()
        ServerService.hideProgressView()
        jobsListData = response as! JSON
        print(jobsListData)
        noJobsLabel.text = jobsListData["Message"].stringValue
        
        if jobsListData["MessageStatus"].intValue == 1
        {
            jobsCategoryDropDown.setTitle(jobsListData["JobCategoryByDivision"].stringValue, for:.normal)
            jobCategoryType = jobsListData["JobCategoryByDivision"].stringValue
            for job in 0..<10
            {
                
                let rJob = ReferAFriend.init(title:jobsListData["JobsList"][job]["JobTitle"].stringValue, jobType: jobsListData["JobsList"][job]["JobType"].stringValue, categoryType: jobsListData["JobsList"][job]["JobCategory"].stringValue, orderId: jobsListData["JobsList"][job]["OrderId"].stringValue, imageUrl: jobsListData["JobsList"][job]["PostingImgUrl"].stringValue, address: jobsListData["JobsList"][job]["City"].stringValue+","+jobsListData["JobsList"][job]["State"].stringValue, jobId:"")
                jobs.append(rJob)
            }
    ...
    func getJobsList()
    {
        if ConnectionCheck.isConnectedToNetwork()
        {
            ...
            print(params)
            ServerService.referalGetJobCategoryList(self, params:params, method:"POST", accessToken:Constants.Token, acces:true, callBack:self.getresponseForJobsList(response:))
        }
        else
        {
            ServerService.hideProgressView()
    ...
    func getresponseForJobsList(response:AnyObject)->()
    {
        ...
        if jobsListData["MessageStatus"].intValue == 1
        {
            jobsCategoryDropDown.setTitle(jobsListData["JobCategoryByDivision"].stringValue, for:.normal)
            jobCategoryType = jobsListData["JobCategoryByDivision"].stringValue
            for job in 0..<jobsListData["JobsList"].arrayValue.count
            {
    ...
    func getTypeOfJobs()
    {
        ...
        {
            ServerService.showActivityIndicatory(uiView:self.view)
            let params:[String:Any] = ["DivisionId":UserDefaults.standard.object(forKey: "dID") as! String,"IsContract":contract,
                                       "IsContractToHire":contractToHire,
                                       "IsDirectHire":direct,"JobCategory":jobCategoryType]
            print("********typeParams******",params)
            ServerService.referalGetJobType(self, params:params, method:"POST", accessToken:Constants.Token, acces:true, callBack:self.getresponseForJobsListFoType(response:))
        }
        else
        {
            ServerService.hideProgressView()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        {
            let dvc = segue.destination as! ReferAFriendJobDetailsViewController
...
//END OF CLASS ReferFriendViewController
...
extension ReferFriendViewController:UITableViewDelegate
{
...
extension ReferFriendViewController:UITableViewDataSource
{
