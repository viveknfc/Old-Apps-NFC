...
@objc public class DateTimePicker: UIView {
    ...
    @objc func methodOfReceivedNotification(notification: Notification){
        ...
        if let minute = self.components.minute {
            var expectedRow = minute / self.timeInterval.rawValue
            if self.timeInterval == .default {
                expectedRow = expectedRow == 0 ? 120 : expectedRow + 60 // workaround for issue when minute = 0
            }
            if self.timeInterval == .default {
                self.components.minute = (expectedRow - 60)%60
            } else {
                self.components.minute = expectedRow * self.timeInterval.rawValue
            }
        }
        if let selected = self.calendar.date(from: self.components) {
            if selected.compare(self.minimumDate) == .orderedAscending {
                self.selectedDate = self.minimumDate
                self.resetTime()
            } else {
    ...
    func resetTime() {
        ...
        if let hour = components.hour {
            ...
            if is12HourFormat == false && isAmPm {
                if hour < 12 {
                    expectedRow = hour + 11
                } else {
                    expectedRow = hour - 1
...
extension DateTimePicker: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == amPmTableView {
            ...
            else if is12HourFormat == false && isAmPm
            {
                cell.textLabel?.text = String(format: "%02i", (indexPath.row % 12) + 1)
            }
            else {
