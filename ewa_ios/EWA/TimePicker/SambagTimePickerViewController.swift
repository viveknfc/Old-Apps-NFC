...
public class SambagTimePickerViewController: UIViewController {
    ...
    var strip3: UIView!
    var minutes:[Int]!
    var timeMintutes:[String]!
    var meridians:[String]!
    ...
    public override func loadView() {
        ...
        minuteWheel.items = timeMintutes
        ...
        {
            let c = minutes.enumerated().min( by: { abs($0.1 - minute) < abs($1.1 - minute)})!
            print(c.element)
            minuteWheel.selectedIndexPath = IndexPath(row:c.offset, section: 0)
        }
    ...
    @objc func didTapOkay() {
        var time = SambagTimePickerResult()
        time.hour = hourWheel.selectedIndexPath.row + 1
        time.minute = Int(minuteWheel.items[minuteWheel.selectedIndexPath.row])! //minuteWheel.selectedIndexPath.row
        if meridians.count>0
        {
            time.meridian = .no
        }
